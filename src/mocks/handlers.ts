...
export const handlers = [
  ...
  http.get("/orgs", ({ request }) => {
    ...
    const orgs = id ? db.orgs.filter((o) => o.id === id) : db.orgs;
    return HttpResponse.json(orgs);
  }),

  http.post("/orgs", async ({ request }) => {
    const newOrg = (await request.json()) as any;
    return HttpResponse.json({ id: String(Date.now()), ...newOrg }, { status: 201 });
  }),

  http.post("/users", async ({ request }) => {
    const newUser = (await request.json()) as any;
    return HttpResponse.json({ id: String(Date.now()), ...newUser }, { status: 201 });
  }),
];
